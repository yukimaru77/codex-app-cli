...
test("rewrites only IAB session selection to use the conversation ID", () => {
  ...
  assert.match(result.source, /t\.partition=Qs\(o\),r\.session=e\(o\)/);
  ...
  assert.match(result.source, /fromPartition\(Qs\(t\)\)/);
  ...
  assert.match(result.source, /Le\(_\.conversationId,_\.browserTabId\)/);
  assert.doesNotMatch(result.source, /t\.partition=Qs\(`app`\)/);
});
