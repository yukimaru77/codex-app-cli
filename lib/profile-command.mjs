...
export function runProfileCommand(
  ...
  if (subcommand === 'status') {
    ...
    return {
      pids,
      runtimeActive: pids.some((pid) => events.some((event) =>
        event.pid === pid && event.event === 'main-bundle-patched-in-memory')),
      latestRuntimeEvent: events.findLast((event) =>
        event.event === 'main-bundle-patched-in-memory') ?? null,
